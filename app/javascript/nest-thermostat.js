...
(function($, factory) {'use strict';
	...
	window.Thermostat = function(thermostatId, userHash)
	{
			...
			this.config.set = {
				temp: function() {
					$.ajax({ 
						...
						url: '/' + userHash + '/device/' + self.device.device + '/hvac/target_temp',
					    data: JSON.stringify({value:self.device.hvac['target_temp'], units:self.device.hvac['temp_scale']})
					});
				}, hum: function() {
					$.ajax({ 
						type: "PUT", contentType: "application/json",
						url: '/' + userHash + '/device/' + self.device.device + '/humidity/target_humidity',
